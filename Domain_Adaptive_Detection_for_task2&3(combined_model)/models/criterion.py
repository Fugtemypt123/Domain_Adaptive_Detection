...
class SetCriterion(nn.Module):
    ...
    def loss_domains(self, domain_predicts, domain_label):

        loss_GAN = 0.0

        for domain_predict in domain_predicts:
            source_predict = domain_predict[:, 0]
            target_predict = domain_predict[:, 1]
            loss_GAN += -torch.sum(domain_label * torch.log(source_predict + 1e-8) + (1 - domain_label) * torch.log(target_predict + 1e-8))

        losses = {'loss_domain': loss_GAN}
        return losses
    ...
    def forward(self, out, annotations=None, domain_label=None, enable_mae=False):
        ...
        # Filter annotations based on 'domain' parameter
        # filtered_indices = [(j, i) for t, (j, i) in zip(annotations, indices) if t['domain'][0] == 1]
        # filtered_annotations = [t for t, (_, i) in zip(annotations, indices) if t['domain'][0] == 1]
        # filtered_num_boxes = sum(len(t["labels"]) for t in annotations if t['domain'][0] == 1)
        # filtered_num_boxes = torch.as_tensor([filtered_num_boxes], dtype=torch.float, device=next(iter(out.values())).device)
        # if is_dist_avail_and_initialized():
        #     torch.distributed.all_reduce(filtered_num_boxes)
        # filtered_num_boxes = torch.clamp(filtered_num_boxes / get_world_size(), min=1).item()
        

        # # loss = self.coef_class * loss_dict['loss_ce'] + self.coef_boxes * loss_dict['loss_bbox'] + self.coef_giou * loss_dict['loss_giou']

        # if not filtered_indices:
        #     # No annotations with 'domain' equal to 1, return zero loss
        ...
        #     loss = (self.coef_class * loss_dict['loss_ce'] + self.coef_boxes * loss_dict['loss_bbox'] + self.coef_giou * loss_dict['loss_giou']) * annotations[0]['domain'].item()
        #     # print(f"zero loss = {loss}")

        # else:
        #     loss_dict.update(self.loss_class(out['pred_logits'], filtered_annotations, filtered_indices, filtered_num_boxes))
        #     loss_dict.update(self.loss_boxes(out['pred_boxes'], filtered_annotations, filtered_indices, filtered_num_boxes))
        #     loss_dict.update(self.loss_giou(out['pred_boxes'], filtered_annotations, filtered_indices, filtered_num_boxes))
        #     loss = self.coef_class * loss_dict['loss_ce'] + self.coef_boxes * loss_dict['loss_bbox'] + self.coef_giou * loss_dict['loss_giou']
        #     # print(f"loss = {loss}")

        # domain_values = torch.tensor([t['domain'].item() for t in annotations]).to(loss.device)

        # if domain_label == True:
        #     loss_dict.update(self.loss_domains(out['domain_predict'], domain_values))
        #     total_loss = loss + loss_dict['loss_domain'] * self.coef_domain_bac
        #     # print(f"loss is {loss}, GAN_loss is {loss_GAN}")
            
        #     return total_loss, loss_dict
        

        if domain_label != None:
            loss_dict.update(self.loss_domains(out['domain_predict'], domain_label))
            ...
            if domain_label == 0:
                ...
                loss = (self.coef_class * loss_dict['loss_ce'] + self.coef_boxes * loss_dict['loss_bbox'] + self.coef_giou * loss_dict['loss_giou']) + loss_dict['loss_domain'] * self.coef_domain_bac

            elif domain_label == 1:
                loss = loss_dict['loss_domain'] * self.coef_domain_bac
            # loss = loss_dict['loss_domain'] * self.coef_domain_bac
