...
def train_one_epoch_standard(args,
    ...
    target_images, target_masks, target_annotations = target_fetcher.next()
    # Training statistics
    ...
    for i in range(total_iters):
        # Forward
        source_out = model(source_images, source_masks)
        #######################################################
        source_loss, source_loss_dict = criterion(source_out, source_annotations, domain_label=0)

        target_out = model(target_images, target_masks)
        target_loss, target_loss_dict = criterion(target_out, target_annotations, 1)
        ...
        if is_main_process() and (i + 1) % print_freq == 0:
            print('Training epoch ' + str(epoch) + ' : [ ' + str(i + 1) + '/' + str(total_iters) + ' ] ' +
